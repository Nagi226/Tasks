...
const quantitiesByCategories = (products) => {
  return products.reduce(function(qtyByCategories, elem) {
     const quantity = elem.quantity;
     const category = elem.category;

     if(qtyByCategories[category] != undefined){
        qtyByCategories[category] = qtyByCategories[category] + quantity;
     }
     else
     {
      qtyByCategories[category] = quantity;
     }
